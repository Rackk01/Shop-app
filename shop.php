...
if (!isset($_GET['ci']) && !isset($_GET['cc']) && !isset($_GET['tx'])) {
    /*
    ci --> Caetgory Id
    cc --> Caetgory Concepto | Denominación/Nombre
    tx --> Texto para la búsqueda
    is --> Id Special ==> Es el id del estado de los productos especiales
    */
    $tit = 'Ooops!';
...
if (isset($_GET['ci']) && isset($_GET['tx'])) {
    $dataArrayProductos =
        getAllProductsOfCategoryWithText(trim($_GET['ci']), trim($_GET['tx']), $orderBy);
    $amount = count(json_decode($dataArrayProductos, true));
} else if (isset($_GET['ci'])) {
    if (isset($_GET['is'])) {
        // Viene de shop-specials.php
        $dataArrayProductos = getAllProductsOfCategoryForAnSpecialState(trim($_GET['ci']), trim($_GET['is']), $orderBy);
        $amount = count(json_decode($dataArrayProductos, true));
    } else {
        $dataArrayProductos = getAllProductsOfCategory(trim($_GET['ci']), $orderBy);
        $amount = count(json_decode($dataArrayProductos, true));
    }
} else if (isset($_GET['tx'])) {
    $dataArrayProductos = getAllProductsWithTxt(trim($_GET['tx']), $orderBy);
    // echo $dataArrayProductos;
    // return;
    if ($dataArrayProductos == '0' || $dataArrayProductos == 0) {
        $amount = 0; //count(json_decode($dataArrayProductos, true));
    } else {
        $amount = count(json_decode($dataArrayProductos, true));
    }

    $dataArrayAmountForCategories = getAmountOfCategories(trim($_GET['tx']), $orderBy);
                    ...
                    <div class="row align-items-center">
                        <div class="col-xl-5">
                            <h4 class="mb-5">
                                ...
                                if (isset($_GET['cc'])) {
                                    if (isset($_GET['is'])) {
                                        echo 'Especiales: ' . ucfirst(strtolower(trim($_GET['cc'])));
                                    } else {
                                        echo ucfirst(strtolower(trim($_GET['cc'])));
                                    }
                                } else if (isset($_GET['tx'])) {
                                    echo 'Resultados para: ' . ucfirst(strtolower(trim($_GET['tx'])));
                                }
                            ...
                            <!-- <div class="breadcrumb">
                                ...
                                ?>
                                    <span></span>
                                <?php
                                    echo 'Texto: ' . ucfirst(strtolower(trim($_GET['tx'])));
                                }
                                ?>
                            </div> -->
                        </div>
                    ...
                    <div class="page-header breadcrumb-wrap">
                        <div class="container">
                            <div class="breadcrumb">
                                ...
                                ?>
                                        <span></span>
                                        <a href="shop?tx=<?php echo $_GET['tx']; ?>&ci=<?php echo $dato['rubro']; ?>&cc=<?php echo $dato['concepto']; ?>">
                                            <?php
